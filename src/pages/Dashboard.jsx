...
function ChartCard({ title, badge, children, className = '' }) {
  return (
    <div className={`bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-200 p-6 flex flex-col gap-4 ${className}`}>
...
function PipelineChart() {
  const counts = PIPELINE_STAGES.map((s) => ({
    ...s, count: MOCK_CASES.filter((c) => c.status === s.key).length,
  }))
    ...
    <div className="flex flex-col gap-2.5">
      {counts.map((s) => (
        <div key={s.key} className="flex items-center gap-3">
          <span className="text-sm text-gray-500 w-16 shrink-0 text-right leading-tight">{s.label}</span>
          ...
          <span className="text-sm font-semibold text-gray-600 w-10 shrink-0 text-right whitespace-nowrap">{s.count} 件</span>
        </div>
      ))}
    </div>
  )
...
function FunnelChart({ data, labelWidth = 'w-16' }) {
      ...
      {data.map((s, i) => {
        ...
        return (
          <div key={s.stage} className="flex items-center gap-3">
            <span className={`text-sm text-gray-500 ${labelWidth} shrink-0 text-right leading-tight`}>{s.stage}</span>
            <div className="flex-1 h-7 bg-gray-50 rounded-full overflow-hidden">
              <div className="h-full rounded-full transition-all duration-500 flex items-center justify-end pr-3"
                ...
                {rate && <span className="text-xs font-semibold text-white whitespace-nowrap">→ {rate}%</span>}
              </div>
            </div>
            <span className="text-sm font-semibold text-gray-600 w-10 shrink-0 text-right whitespace-nowrap">{s.count} 件</span>
          </div>
...
function AppealFunnelChart()     { return <FunnelChart data={MOCK_APPEAL_FUNNEL}     labelWidth="w-20" /> }
function RetainerFunnelChart()   { return <FunnelChart data={MOCK_RETAINER_FUNNEL}   labelWidth="w-24" /> }
...
export default function Dashboard() {
  ...
  const nearestJudgment = judgmentPendingCases.map(c => c.renewalDeadline).sort()[0]
  const nearestRetainer = retainerPendingCases.map(c => c.renewalDeadline).sort()[0]

  const consultHoursTrend = (() => {
    const d = MOCK_KPI.consultationHours - MOCK_KPI.consultationHoursLastMonth
    if (d === 0) return null
    const pct = Math.round(Math.abs(d / MOCK_KPI.consultationHoursLastMonth) * 100)
    return { label: `${d > 0 ? '+' : '-'}${pct}%`, up: d > 0, positive: d > 0 }
  })()
    ...
    <div>
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          {/* ── 區塊 1：客戶開發漏斗（諮詢 → 委任 → 進行中） ─────────────── */}

          {/* 諮詢案件：配對轉換指標 */}
          <ChartCard title="諮詢案件" className="col-span-2">
            <CasePieChart data={consultData} />
          </ChartCard>
          ...
          <KpiCard label="諮詢時數"   value={MOCK_KPI.consultationHours} unit="hr" sub="上月比較" trend={consultHoursTrend} />

          {/* 確定委任案件：配對結案指標 */}
          <ChartCard title="確定委任案件" className="col-span-2">
            <CasePieChart data={retainedData} />
          </ChartCard>
          <KpiCard label="預計結案" value={MOCK_KPI.plannedCloseThisMonth} unit="件" sub="上月比較" trend={calcTrend(MOCK_KPI.plannedCloseThisMonth, MOCK_KPI.plannedCloseLastMonth, ' 件')} />
          <KpiCard label="實際結案" value={MOCK_KPI.closedThisMonth}       unit="件" sub="上月比較" trend={calcTrend(MOCK_KPI.closedThisMonth, MOCK_KPI.closedLastMonth, ' 件')} />

          {/* 進行中案件：配對帳務指標 */}
          <ChartCard title="進行中案件" badge="即時" className="col-span-2">
          ...
          <KpiCard label="結案已逾期"   value={overdueCount}      unit="件" variant={overdueCount      > 0 ? 'danger'  : undefined} />
          <KpiCard label="結案即將逾期" value={warningCount}      unit="件" variant={warningCount      > 0 ? 'warning' : undefined} />
          ...
          <KpiCard label="上訴待委任"     value={judgmentPendingCases.length} unit="件"
            sub={nearestJudgment ? `最近到期 ${nearestJudgment}` : undefined}
            variant={judgmentPendingCases.length > 0 ? 'warning' : undefined} className="col-span-2" />
          <KpiCard label="常年合約待續約" value={retainerPendingCases.length} unit="件"
            sub={nearestRetainer ? `最近到期 ${nearestRetainer}` : undefined}
            variant={retainerPendingCases.length > 0 ? 'warning' : undefined} className="col-span-2" />

          {/* ── 區塊 3：期限明細 ───────────────────────────────────────────── */}

          <ChartCard title="結案期限明細" className="col-span-2">
                ...
                return (
                  <div key={c.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                    ...
                    <div className="flex items-center gap-2 shrink-0 ml-4">
                      ...
                      <span className={`text-sm font-semibold px-2.5 py-1 rounded-full ${chipClass}`}>
          ...
          <ChartCard title="下一個事件時間點" className="col-span-2">
            <div className="overflow-y-auto max-h-[320px] flex flex-col divide-y divide-gray-50 pr-1">
              {allEvents.map((item) => {
                ...
                return (
                  <div key={item.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                    ...
                    <div className="flex items-center gap-2 shrink-0 ml-4">
                      ...
                      <span className={`text-sm font-semibold px-2.5 py-1 rounded-full ${daysLeftColor(item.daysLeft)}`}>
          ...
          <ChartCard title="案件流程分布" className="col-span-2">
            <PipelineChart />
          </ChartCard>
          <ChartCard title="客戶轉換漏斗" className="col-span-2">
            <ConversionFunnelChart />
          </ChartCard>
          <ChartCard title="上訴未委任追蹤" className="col-span-2">
            <AppealFunnelChart />
          </ChartCard>
          <ChartCard title="常年合約續約追蹤" className="col-span-2">
            <RetainerFunnelChart />
          </ChartCard>

          {/* ── 區塊 5：全寬分析 ───────────────────────────────────────────── */}

          <ChartCard title="各案件類型平均結案週期" className="col-span-2 md:col-span-4">
            <CycleDaysChart />
          </ChartCard>
